"""
Unified Emerging Appreciation Score + Dashboard JSON builder
-------------------------------------------------------------
Config-driven replacement for jc_heights_emerging.py, hoboken_emerging.py, and
weehawken_emerging.py.  Reads {prefix}_cleaned.csv + {prefix}_block.csv from
data_dir, writes {prefix}_blocks_v2.csv and dashboard_data_v2.json.

Usage:
    python -m pipeline.emerging --city jc_heights
    python -m pipeline.emerging --city hoboken --data-dir data/processed/hoboken
    python -m pipeline.emerging --city all
"""
...
def normalize_list(values, invert=False):
    """Percentile-rank normalization [0, 1]. Robust to outliers."""
...
def load_geocache(data_dir):
    geocache = {}
    path = data_dir / "address_geocoded.csv"
    if not path.exists():
        return geocache
...
def load_open_houses(data_dir):
    open_house_lu = {}
    path = data_dir / "open_houses.csv"
    if not path.exists():
        return open_house_lu
    with open(path, encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            mls = row.get("MLS #", "").strip()
            d = row.get("Tour/Open House - Start Date", "").strip()
            s = row.get("Tour/Open House - Start Time", "").strip()
            e = row.get("Tour/Open House - End Time", "").strip()
            if mls and d:
                open_house_lu.setdefault(mls, []).append(
...
def run_emerging(city_key, data_dir=None):
    """Run the emerging-score + dashboard JSON pipeline for one city."""
    ...
    print(f"\n{'='*60}")
    print(f"  Emerging: {cfg['name']} ({city_key})")
    print(f"  Data dir: {data_dir}")
    print(f"{'='*60}")

    # ── Load cleaned + block outputs ────────────────────────────────────────
    cleaned_path = data_dir / f"{prefix}_cleaned.csv"
    block_path = data_dir / f"{prefix}_block.csv"
    ...
    geocache = load_geocache(data_dir)
    print(f"Geocache: {len(geocache)} addresses")

    census_lu = load_census(data_dir)
    print(f"Census ZIPs: {len(census_lu)}")

    open_house_lu = load_open_houses(data_dir)
    print(f"Open houses: {sum(len(v) for v in open_house_lu.values())} sessions / "
          f"{len(open_house_lu)} listings")

    # ── Constants ───────────────────────────────────────────────────────────
    today = date.today()
    cutoff_recent = today - timedelta(days=365)
    cutoff_mid = today - timedelta(days=365 * 2)

    RADIUS_MIN_COMPS = 5
    RADIUS_MIN_BLOCK_SALES = 10
    MIN_RECENT = 3
    ...
    spatial_comps = []
    for p in sold:
        geo = geocache.get(p.get('address', ''))
        if geo and p.get('_psf'):
            spatial_comps.append(
                (geo['lat'], geo['lon'], p['_psf'], p.get('property_type', ''))
            )
    print(f"Spatial comp index: {len(spatial_comps)} geocoded sold properties")

    def radius_median_psf(lat, lon, prop_type=''):
        same_type = [psf for clat, clon, psf, ctype in spatial_comps
                     if ctype == prop_type
                     and _haversine_miles(lat, lon, clat, clon) <= radius_miles]
        if len(same_type) >= RADIUS_MIN_COMPS:
            return statistics.median(same_type), len(same_type)
        all_nearby = [psf for clat, clon, psf, _ in spatial_comps
    ...
    # ── Zip-level median PSF + block→zip lookup ─────────────────────────────
    ...
    block_recent = defaultdict(list)
    block_mid = defaultdict(list)
    block_prior = defaultdict(list)
    ...
    for p in sold:
        ...
        if d >= cutoff_recent:
            ...
            if dom is not None:
                block_dom_recent[bk].append(dom)
            if stl is not None:
                block_stl_recent[bk].append(stl)
        elif d >= cutoff_mid:
            block_mid[bk].append(psf)
        else:
            block_prior[bk].append(psf)
            if dom is not None:
                block_dom_prior[bk].append(dom)
            if stl is not None:
                block_stl_prior[bk].append(stl)

    # ── Emerging signals per block ──────────────────────────────────────────
    emerging_raw = {}
    for bk in set(p['block'] for p in sold):
        ...
        zip_code = block_zip.get(bk, default_zip)
    ...
    weights = {'underval': 0.30, 'acceleration': 0.30,
               'dom_tightening': 0.20, 'demand_signal': 0.20}

    underval_n = normalize_list([b['underval'] for b in bk_list])
    accel_n = normalize_list([b['acceleration'] for b in bk_list])
    dom_n = normalize_list([b['dom_tightening'] for b in bk_list], invert=True)
    ...
    for i, b in enumerate(bk_list):
        ...
        if (b.get('underval') is not None and b.get('acceleration') is not None
            ...
            interaction_bonus = 5
        b['emerging_score'] = (min(100, base_score + interaction_bonus)
    ...
    for bk in set(list(established.keys()) + list(emerging_lu.keys())):
        ...
        merged_blocks.append({
            ...
            'n_sales': (flt(est.get('n_sales'))
                        or emg.get('n_recent', 0) + emg.get('n_prior', 0)),
            ...
            'acceleration': (round(emg['acceleration'] * 100, 1)
                             if emg.get('acceleration') is not None else None),
            'underval_pct': (round(emg['underval'] * 100, 1)
                             if emg.get('underval') is not None else None),
    ...
    for p in props:
        ...
        rad_psf, rad_n = None, 0
        psf_source = 'block'
        is_zip_block = r.get('block', '').startswith('zip_')
        if ((not block_psf or block_n < RADIUS_MIN_BLOCK_SALES or is_zip_block)
                and r['lat'] and r['lon']):
            rad_psf, rad_n = radius_median_psf(r['lat'], r['lon'],
                                               r.get('property_type', ''))
            if rad_psf:
                radius_used += 1
                psf_source = 'radius'
        ...
        r['block_median_psf'] = ((rad_psf if (is_zip_block and rad_psf) else None)
                                 or block_psf or rad_psf)
        r['radius_median_psf'] = rad_psf
        r['radius_n_comps'] = rad_n if rad_psf else None
        r['psf_source'] = psf_source
        enriched_props.append(r)

    print(f"PSF source — block: {len(enriched_props) - radius_used - radius_skipped}  "
          f"radius: {radius_used}  none: {radius_skipped}")

    # ── Write blocks_v2 CSV ─────────────────────────────────────────────────
    blocks_v2_path = data_dir / f"{prefix}_blocks_v2.csv"
    with open(blocks_v2_path, 'w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=merged_blocks[0].keys())
        w.writeheader()
        w.writerows(merged_blocks)
    print(f"Wrote {len(merged_blocks)} blocks -> {blocks_v2_path}")
    ...
    trends_out = {
        bk: {yr: round(statistics.median(psfs), 0)
             for yr, psfs in sorted(yrs.items()) if psfs}
    ...
    out_json = data_dir / f"{prefix}_dashboard_data_v2.json"
    with open(out_json, 'w', encoding='utf-8') as f:
        json.dump(dashboard, f, separators=(',', ':'))
    sz = len(json.dumps(dashboard)) / 1024
    print(f"Dashboard JSON ({sz:.0f} KB) -> {out_json}")

    # ── Top-10 report ───────────────────────────────────────────────────────
    print(f"\nTOP 10 EMERGING BLOCKS — {cfg['name']}:")
    print(f"{'Rank':<5} {'Block':<24} {'Emg':<6} {'Est':<6} {'Accel%':<9} "
          f"{'Underval%':<11} {'PSF Recent':<12} {'DOM tight'}")
    print("-" * 90)
    for b in ranked_emerging[:10]:
        est_sc = established.get(b['block'], {}).get('appreciation_score') or 0
        print(
            f"{b['emerging_rank']:<5} {b['block'].replace('block_','')[:23]:<24} "
            f"{b['emerging_score']:<6} "
            f"{str(round(float(est_sc))):<6} "
    ...
    return {
        ...
        'ranked_emerging': ranked_emerging,
        'dashboard_path': out_json,
    }


def _default_data_dir(city_key):
    root = Path(__file__).resolve().parent.parent
    if city_key == "jc_heights":
        return root / "data" / "processed"
    return root / "data" / "processed" / city_key.replace("_", "")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--city", required=True,
                    help="City key (jc_heights, hoboken, weehawken) or 'all'")
    ap.add_argument("--data-dir", type=Path, default=None,
                    help="Directory with cleaned CSVs & enrichments "
                         "(default: data/processed[/<city>])")
    args = ap.parse_args()
    ...
    for key in targets:
        if key not in CITY_CONFIGS:
            ...
            sys.exit(1)
        dd = args.data_dir or _default_data_dir(key)
        run_emerging(key, dd)
