"""
Hoboken — Emerging Appreciation Score
--------------------------------------
...
Hoboken-specific adaptations vs. jc_heights_emerging.py:
  - Reads hoboken_cleaned.csv / hoboken_block.csv
  - RADIUS_MILES reduced to 0.20 (Hoboken is ~1 sq mile, tighter comps)
  - Writes hoboken_dashboard_data.json
"""
...
# ── LOAD DATA ────────────────────────────────────────────────────────────
with open('hoboken_cleaned.csv') as f:
    props = list(csv.DictReader(f))

with open('hoboken_block.csv') as f:
...
# Hoboken is ~1 sq mile so tighten the radius vs. the JC script
...
for bk in set(p['block'] for p in sold):
    ...
    zip_code = block_zip.get(bk, '07030')
    ...
    zip_thresh   = zip_med or 900   # Hoboken baseline higher than JC
...
with open('hoboken_blocks_v2.csv', 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=merged_blocks[0].keys())
    w.writeheader(); w.writerows(merged_blocks)
print(f"Wrote {len(merged_blocks)} blocks -> hoboken_blocks_v2.csv")
...
with open('hoboken_dashboard_data.json','w') as f:
