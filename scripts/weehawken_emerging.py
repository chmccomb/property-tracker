"""
Weehawken — Emerging Appreciation Score
--------------------------------------
Adds a forward-looking "Emerging" score to each block alongside the
existing "Established" score.  Writes final dashboard-ready JSON.

Weehawken-specific adaptations vs. jc_heights_emerging.py:
  - Reads weehawken_cleaned.csv / weehawken_block.csv
  - RADIUS_MILES reduced to 0.20 (Weehawken is ~1 sq mile, tighter comps)
  - Writes weehawken_dashboard_data.json
"""
...
# ── LOAD DATA ────────────────────────────────────────────────────────────
with open('weehawken_cleaned.csv') as f:
    props = list(csv.DictReader(f))

with open('weehawken_block.csv') as f:
...
# ── OPEN HOUSE SCHEDULE ───────────────────────────────────────────────────
# Keyed on MLS # → list of {date, start, end} dicts.
# File produced by copying Tour_Open_House_Thumbnail.csv from Paragon export.
open_house_lu: dict = {}
...
sold = []
for p in props:
    if p.get('status') not in ('SOLD',): continue
...
# Weehawken is ~1 sq mile so tighten the radius vs. the JC script
RADIUS_MILES           = 0.20   # ~3-minute walk; keeps comps in the same building cluster
RADIUS_MIN_COMPS       = 5
...
for bk in set(p['block'] for p in sold):
    ...
    zip_code = block_zip.get(bk, '07086')
    ...
    zip_thresh   = zip_med or 700   # Weehawken median PSF baseline
...
for bk in all_block_keys:
    ...
    merged_blocks.append({
        'block':            bk,
        'block_raw':        bk.replace('block_','').replace('complex_','').replace('zip_',''),
...
with open('weehawken_blocks_v2.csv', 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=merged_blocks[0].keys())
    w.writeheader(); w.writerows(merged_blocks)
print(f"Wrote {len(merged_blocks)} blocks -> weehawken_blocks_v2.csv")
...
with open('weehawken_dashboard_data.json','w') as f:
...
print(f"{'Rank':<5} {'Block':<28} {'Emg':<6} {'Est':<6} {'Accel%':<9} {'Underval%':<11} {'PSF Recent':<12} {'DOM tight'}")
print("-"*80)
for b in ranked_emerging[:10]:
    print(f"{b['emerging_rank']:<5} {b['block'].replace('block_','').replace('complex_',''):<28} "
