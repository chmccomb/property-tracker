"""
Hoboken MLS Data Cleaning & Appreciation Scoring
-------------------------------------------------
Input:  Testing.csv  (Paragon MLS custom export, Hoboken)
Output: hoboken_cleaned.csv   — cleaned, enriched dataset
        hoboken_scored.csv    — per-property appreciation scores
        hoboken_block.csv     — block-level market summary

Hoboken-specific adaptations vs. jc_heights_clean.py:
  - block_key() falls back to Complex Name when Block is missing (~14% of rows)
  - ZIP normalization: '07030-XXXX' extended zips → '07030'
  - Parking regex: adds Hoboken-specific negatives (optional/rental/off-site)
    because Hoboken listings often mention parking as an available upsell,
...
OUT_CLEAN    = "hoboken_cleaned.csv"
OUT_SCORED   = "hoboken_scored.csv"
OUT_BLOCK    = "hoboken_block.csv"
...
def block_key(row):
    """Micro-market key for Hoboken.

    Priority:
      1. Block + Street  (when enough data, same as JC logic)
      2. Block only
      3. Complex Name    (Hoboken-specific: ~14% of rows have no Block)
      4. ZIP             (last resort)
...
for row in raw:
    ...
    # Hoboken-specific parking logic:
    #   Many Hoboken listings mention parking as an available *upsell* rather
    ...
    # Hoboken-specific negatives: parking is available to rent / off-site / optional
    _park_neg = bool(re.search(
        r'\b(no\s+parking|street\s+parking\s+only|optional\s+parking|'
        r'rental\s+parking|off-?site\s+parking)\b', _rem))
