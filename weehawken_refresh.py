#!/usr/bin/env python3
"""
Weehawken — Data Refresh Pipeline
---------------------------------
Runs the full Weehawken pipeline whenever a new Paragon MLS CSV export is
available.  Mirrors hoboken_refresh.py but wired to the Weehawken scripts
and data dir.

Usage:
    python weehawken_refresh.py --csv path/to/new_export.csv
    python weehawken_refresh.py --csv path/to/new_export.csv --enrich   # re-run MOD-IV
    python weehawken_refresh.py --csv path/to/new_export.csv --deploy   # git push after

Steps:
    1. Copy the input CSV to data/processed/weehawken/Testing.csv
    2. Run geocode.py --city ", Weehawken, NJ"  (instant for cached addresses)
    3. Run enrich_transit.py                    (pure math, instant)
    4. (Optional, --enrich) Run enrich_modiiv.py --muni 0911
    5. Run weehawken_clean.py
    6. Run weehawken_emerging.py
    7. Embed the new JSON into dashboard/weehawken_dashboard.html
    8. (Optional, --deploy) Copy to docs/weehawken.html, git add/commit/push
"""
...
DATA_DIR      = ROOT / "data" / "processed" / "weehawken"
DASHBOARD     = ROOT / "dashboard" / "weehawken_dashboard.html"

CLEAN_SCRIPT    = SCRIPTS_DIR / "weehawken_clean.py"
EMERGING_SCRIPT = SCRIPTS_DIR / "weehawken_emerging.py"
...
OUTPUT_JSON     = DATA_DIR / "weehawken_dashboard_data.json"
DOCS_DIR        = ROOT / "docs"
DOCS_DASHBOARD  = DOCS_DIR / "weehawken.html"
...
def run_enrichment() -> None:
    ...
    run_step(
        "MOD-IV assessment enrichment (Weehawken)",
        [sys.executable, str(MODIIV_SCRIPT), "--muni", "0911"],
        cwd=DATA_DIR,
    )


def deploy_to_github(timestamp: str) -> None:
    """Copy dashboard to docs/weehawken.html and push to GitHub Pages."""
    print(f"\n{'─'*60}")
    ...
    print(f"✓ Copied dashboard → docs/weehawken.html ({size_kb:.0f} KB)")
    ...
    _git(["add", "docs/weehawken.html", "dashboard/weehawken_dashboard.html"])
    commit_msg = f"weehawken: refresh data {timestamp}"
    rc = _git(["commit", "-m", commit_msg])
    if rc == 0:
        push_rc = _git(["push"])
        if push_rc == 0:
            print("✓ Pushed to GitHub — Weehawken dashboard live")
        else:
...
def refresh(csv_path: Path, enrich: bool = False, deploy: bool = False) -> None:
    ...
    run_step(
        ...
        [sys.executable, str(GEOCODE_SCRIPT), "--city", ", Weehawken, NJ"],
        cwd=DATA_DIR,
    )
    ...
    run_step(
        "Step 1 — weehawken_clean.py",
        [sys.executable, str(CLEAN_SCRIPT)],
        cwd=DATA_DIR,
    )

    run_step(
        "Step 2 — weehawken_emerging.py",
    ...
    if DASHBOARD.exists():
        ...
        print(f"  ⚠ Dashboard HTML not found at {DASHBOARD} — skipping embed")

    if deploy:
        deploy_to_github(ts)

    print(f"\n{'='*60}")
    print(f"  ✓ Refresh complete — open dashboard/weehawken_dashboard.html")
    print(f"{'='*60}\n")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Weehawken dashboard from a new Paragon MLS CSV export."
    )
    ...
    parser.add_argument("--deploy", action="store_true",
                        help="After refresh, copy to docs/weehawken.html and git push")
